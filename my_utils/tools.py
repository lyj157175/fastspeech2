...
# from get_model import vocoder_infer
...
def get_mask(lengths, max_len=None, device=torch.device("cuda")):
...
def synth_one_sample(targets, predictions, vocoder, model_config, preprocess_config):
    ...
    wav_reconstruction = vocoder_infer(
        mel_target.unsqueeze(0),  # 1, 80, mel_len
        vocoder,
        model_config,
        preprocess_config,
    )[0]
    print('xxxx', wav_reconstruction.shape)
    ...
    print('sss', wav_prediction.shape)
...
def synth_samples(targets, predictions, vocoder, model_config, preprocess_config, path):
    ...
    for wav, basename in zip(wav_predictions, basenames):
        wavfile.write(os.path.join(path, "{}.wav".format(basename[:-1])), sampling_rate, wav)

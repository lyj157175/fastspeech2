...
def evaluate(model, step, configs, vocoder=None):
    ...
    message = "Validation Step {}, " \
              ...
              "Pitch Loss: {:.4f}, " \
              "Energy Loss: {:.4f}, " \
              "Duration Loss: {:.4f}".format(*([step] + [l for l in loss_means]))
    ...
    wavfile.write('output/synth_audio/truth_{}.wav'.format(step), 22050, wav_reconstruction)
    wavfile.write('output/synth_audio/pred_{}.wav'.format(step), 22050, wav_prediction)
    return message
